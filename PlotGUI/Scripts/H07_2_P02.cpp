...
private: // Static Function
	...
	static double PhiPiecewise(double x, int i, int n)
	{
		double h = 1.0 / n;
		double xi = i * h;
	...
	static void SolveIntegralEquation(double ax, double bx, double as, double bs, int n, DVecX& x, DVecX& y)
	{
		...
		for (int i = 0; i <= n; ++i)
		{
			...
			for (int j = 0; j <= n; ++j)
			{
				...
				{
					return PhiPiecewise(x, j, n) - IntergralSimpson([&](double s) -> double
					{
						return PhiPiecewise(s, j, n) * K(x, s);
					}, as, bs, n) * PhiPiecewise(x, i, n);
				}, ax, bx, n);
			}

			b[i] = IntergralSimpson([&](double x) -> double
			{
				return F(x) * PhiPiecewise(x, i, n);
			}, ax, bx, n);
		}
